...
dracoLoader.setDecoderPath( 'https://9positions.com/wp-content/themes/shoptimizer-child-theme/assets/js/decoder/' );
dracoLoader.preload();
...
loader.load(
  ...
  (gltf) => {
    ...
    controls.minDistance = 0.8; // Adjust this value as needed
    controls.maxDistance = 1.05; // Adjust this value as needed
    controls.update();
  ...
  (xhr) => {
    ...
    const cgkit = document.querySelectorAll('button.cgkit-swatch');
    cgkit.forEach( (cg) => { 
      cg.addEventListener("click", (e) => {
          ...
          if (e.target.classList.contains("cgkit-swatch")) {
            ...
            let label =
              e.target.parentElement.parentElement.parentElement.parentNode.previousElementSibling.textContent;
              console.log(label)
            if(label.includes(':')){
            ...
            switch (label) {
                case "Glove Logo":
                  updateColor("9p_logo_main", colors[value]);
                  break;
                case "Glove Logo Outline":
                  updateColor("9p_logo_outline", colors[value]);
                  break;
                case "Thumb Inner Color":
                  updateColor("thumb_inner", colors[value]);
                  break;
                case "Thumb Outer Color":
                  updateColor("thumb_outer", colors[value]);
                  break;
                ...
                case "Index Outer Color":
                  ...
                  break;
                case "Middle Index Color":
                  updateColor("middle-index_connection", colors[value]);
                  break;
                case "Middle Inner Color":
                  //updateColor("middle_inner", colors[value]);
                  ["middle_inner", "middle_middle", "middle_finger"].forEach((material) => {
                        if (materialIsVisible(material)) {
                          updateColor(material, colors[value]);
                        }
                    });
                  break;
                case "Middle Finger Color":
                  updateColor("middle_finger", colors[value]);
                  break;
                case "Middle Outer Color":
                  updateColor("middle_outer", colors[value]);
                  break;
                case "Ring Middle Color":
                  updateColor("ring-middle_connection", colors[value]);
                  break;
                ...
                case "Welt Color":
                  ...
                  break;
                case "Lace Color":
                  ["laces", "web_laces"].forEach((material) => {
                    if (materialIsVisible(material)) {
                      updateColor(material, colors[value]);
                    }
                  });
                ...
                case "Stitching Color":
                  ...
                  if (materialIsVisible("finger_pad")) {
                    updateColor("finger_pad_stitches", colors[value]);
                  }
                  break;
                case "Embroidery Color":
                  if (materialIsVisible("embroidery")) {
...
const updateColor = (id, colorCode) => {
